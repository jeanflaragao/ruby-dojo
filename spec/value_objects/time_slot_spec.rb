...
RSpec.describe TimeSlot do
  ...
  describe 'overlaps?' do
    let(:slot1) { described_class.new(Time.new(2024, 1, 1, 10, 0), Time.new(2024, 1, 1, 11, 0)) }

    it 'returns true when slots overlap' do
      slot2 = described_class.new(Time.new(2024, 1, 1, 10, 30), Time.new(2024, 1, 1, 11, 30))
      expect(slot1.overlaps?(slot2)).to be true
    end

    it 'returns false when slots do not overlap' do
      slot2 = described_class.new(Time.new(2024, 1, 1, 11, 0), Time.new(2024, 1, 1, 12, 0))
  ...
  describe 'formatting' do
    it 'formats with AM/PM' do
      slot = described_class.new(Time.new(2024, 1, 1, 14, 0), Time.new(2024, 1, 1, 15, 30))
