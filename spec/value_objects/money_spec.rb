...
RSpec.describe Money do
  describe 'initialization' do
    ...
    it 'raises error for negative amounts' do
      expect { Money.new(-100, 'USD') }.to raise_error(ArgumentError, /positive/)
    end

    it 'raises error for nil amount' do
      expect { Money.new(nil, 'USD') }.to raise_error(ArgumentError)
    end
  ...
  describe 'equality' do
    ...
    it 'can be used as hash key' do
      ...
      expect(hash[Money.new(100, 'USD')]).to eq('ticket_price')
    end
  end

  describe 'arithmetic' do
    let(:money1) { Money.new(100, 'USD') }
    let(:money2) { Money.new(50, 'USD') }

    describe 'addition' do
      it 'adds two money objects' do
        result = money1 + money2
        expect(result).to eq(Money.new(150, 'USD'))
      end
    ...
    describe 'subtraction' do
      it 'subtracts two money objects' do
        result = money1 - money2
        expect(result).to eq(Money.new(50, 'USD'))
      end
    ...
    describe 'multiplication' do
      it 'multiplies by a number' do
        result = money1 * 3
        expect(result).to eq(Money.new(300, 'USD'))
      end
  ...
  describe 'comparison' do
    it 'compares money values' do
      small = Money.new(50, 'USD')
      large = Money.new(100, 'USD')
