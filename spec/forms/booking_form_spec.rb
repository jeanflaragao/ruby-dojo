...
RSpec.describe BookingForm do
  ...
  describe 'validation' do
    describe 'event_name' do
      ...
      it 'is invalid when event_name is blank' do
        form = described_class.new(seats: '2', ticket_type: 'general', email: 'user@example.com')
        expect(form).not_to be_valid
        expect(form.errors[:event_name]).to include("can't be blank")
      end

      it 'is invalid when event_name is empty string' do
        form = described_class.new(event_name: '', seats: '2', ticket_type: 'general', email: 'user@example.com')
        expect(form).not_to be_valid
      end
    end
    ...
    describe 'seats' do
      it 'is invalid when seats is blank' do
        form = described_class.new(event_name: 'Conference', ticket_type: 'general', email: 'user@example.com')
        expect(form).not_to be_valid
        expect(form.errors[:seats]).to include("can't be blank")
      end

      it 'is invalid when seats is not a number' do
        form = described_class.new(event_name: 'Conference', seats: 'abc', ticket_type: 'general',
                                   email: 'user@example.com')
        expect(form).not_to be_valid
      ...
      it 'is invalid when seats is negative' do
        form = described_class.new(event_name: 'Conference', seats: '-1', ticket_type: 'general',
                                   email: 'user@example.com')
        expect(form).not_to be_valid
      ...
      it 'is invalid when seats is zero' do
        form = described_class.new(event_name: 'Conference', seats: '0', ticket_type: 'general',
                                   email: 'user@example.com')
        expect(form).not_to be_valid
        expect(form.errors[:seats]).to include('must be positive')
      end
      ...
      it 'is invalid when seats exceeds maximum (10)' do
        form = described_class.new(event_name: 'Conference', seats: '11', ticket_type: 'general',
                                   email: 'user@example.com')
        expect(form).not_to be_valid
    ...
    describe 'ticket_type' do
      it 'is valid with vip ticket type' do
        form = described_class.new(event_name: 'Conference', seats: '2', ticket_type: 'vip', email: 'user@example.com')
        expect(form).to be_valid
      end

      it 'is valid with general ticket type' do
        form = described_class.new(event_name: 'Conference', seats: '2', ticket_type: 'general',
                                   email: 'user@example.com')
        expect(form).to be_valid
      end

      it 'is valid with student ticket type' do
        form = described_class.new(event_name: 'Conference', seats: '2', ticket_type: 'student',
                                   email: 'user@example.com')
        expect(form).to be_valid
      end

      it 'is invalid with unknown ticket type' do
        form = described_class.new(event_name: 'Conference', seats: '2', ticket_type: 'premium',
                                   email: 'user@example.com')
        expect(form).not_to be_valid
      ...
      it 'is invalid when ticket_type is blank' do
        form = described_class.new(event_name: 'Conference', seats: '2', email: 'user@example.com')
        expect(form).not_to be_valid
      end
    end
    ...
    describe 'email' do
      ...
      it 'is invalid with improper email format' do
        form = described_class.new(event_name: 'Conference', seats: '2', ticket_type: 'general', email: 'notanemail')
        expect(form).not_to be_valid
    ...
    describe 'multiple errors' do
      it 'collects all validation errors' do
        form = described_class.new(seats: 'abc', ticket_type: 'invalid', email: 'bad')
        expect(form).not_to be_valid
  ...
  describe '#error_messages' do
    it 'returns human-readable error messages' do
      form = described_class.new(seats: 'abc')
      form.valid?
