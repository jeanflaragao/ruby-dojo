...
RSpec.describe TicketType do
  describe 'base class' do
    it 'cannot be instantiated directly' do
      expect { TicketType.new }.to raise_error(NotImplementedError)
    end
  end

  describe VIPTicket do
    let(:base_price) { Money.new(100, 'USD') }
    let(:vip_ticket) { VIPTicket.new(base_price) }
  ...
  describe GeneralTicket do
    let(:base_price) { Money.new(100, 'USD') }
    let(:general_ticket) { GeneralTicket.new(base_price) }
  ...
  describe StudentTicket do
    let(:base_price) { Money.new(100, 'USD') }
    let(:student_ticket) { StudentTicket.new(base_price) }
